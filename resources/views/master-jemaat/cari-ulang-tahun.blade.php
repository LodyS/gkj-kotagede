    ...
    <div class="row justify-content-center">
        <div class="col-lg-12">
            <div class="card">
                ...
                <form action="" method="POST" id="list-ulang-tahun">@csrf
...
<h1 align="center">Hasil Pencarian</h1>
    <div class="media-body">
        <table class="table table-hover" id="daftar-ulang-tahun">
            <thead>
                <tr>
                    <td>No</td>
                    <td>Nama Jemaat</td>
...
$('#list-ulang-tahun #cari').on('click', function (e) {
    e.preventDefault();
        $.ajax({
            type: 'POST',
            url: "search-ulang-tahun",
            data: $("#list-ulang-tahun").serialize(),
            dataType: 'json',
            success: function (data) {
                console.log(data);
                let table = $("#daftar-ulang-tahun tbody");
                let no = 1;
                table.empty();
                $.each(data, function(i, item){

                    var date = new Date(item.tanggal).toDateString("d-m-y");  
                    table.append("<tr><td>"+no+++"</td><td>"+item.nama_jemaat+"</td><td>"+date+"</td><td>"+item.ket+"</td></tr>");
